...
function planningSystemPrompt(submittedCode, verifiedStructure) {
...
game capabilities and which are already done, the canonical object names and
gameplay constants, Nick's real, latest submitted code, and the verified Explorer
structure from every mission he's actually had approved (see below \u2014 together these
are the only source of truth for what already exists in his project; do not use a
mission description, a summary, or your own memory of an earlier mission for that).
Some real things in Nick's project \u2014 folders and Parts he built by clicking in
Studio \u2014 were never typed as code anywhere and will only appear in the verified
structure, not in the submitted code; both sources count as real. Steer toward the
next undone capability, but you are free to split it into a smaller mission, combine
it with setup, or reorder \u2014 there is no fixed mission list to follow.
...
- Grounding rule: every identifier, object, variable, function, property, event,
  folder, or path that this mission's instructions or code treat as already existing
  must literally appear in Nick's submitted code OR his verified Explorer structure
  below. If it doesn't actually appear in either, you may still have Nick use it \u2014
  but only by having this mission's own steps explicitly teach him to create it
  first, introduced as something new, never assumed to already be there. Never carry
  over a name from an earlier mission's description or from what a name "should" be
  if it isn't in the real code or real structure below.
...
=== Nick's real, latest submitted code (ground truth for what already exists) ===
${submittedCode}

=== Nick's verified Explorer structure, from every mission he's actually had approved (ground truth for what already exists in Studio, even if never typed as code) ===
${verifiedStructure}
...
function qaSystemPrompt(submittedCode, verifiedStructure) {
...
nobody checked engine behavior against assumption before it shipped, it happened
a second time with V1-M05 (two buttons left at default Position, so they silently
overlapped and one was unclickable), and it happened a third time with V1-M07
(new code placed inside an earlier function, referencing names that don't exist
in that scope) because the planner assumed what Nick's code looked like instead
of checking it. This call exists so none of these can be skipped again.

Grounding check, in addition to everything else below: for every identifier,
object, variable, function, property, event, folder, or path this mission's
instructions or code treat as already existing, confirm it literally appears in
Nick's real submitted code OR his verified Explorer structure, both given below.
Some real things \u2014 folders and Parts built by clicking in Studio \u2014 only appear in
the verified structure, never in code; that still counts as real. If a name
appears in neither, and the mission's own steps don't explicitly teach Nick to
create it before using it, that is a real issue \u2014 fix it the same way you fix any
other issue found below (add the missing creation step, or correct the assumed
name to one that's actually real below), and list it in issues_found.
    ...
    try {
      ...
      if (path === "/internal/replan" && request.method === "POST") {
        try {
          ...
          return json({ ok: false, error: String(replanErr.message || replanErr) }, 409);
        }
      }
...
async function getVerifiedStructure(env) {
  const rows = await env.DB.prepare(
    "SELECT id, content_json FROM missions WHERE status = 'approved' ORDER BY number ASC"
  ).all();
  const parts = [];
  for (const row of (rows.results || [])) {
    ...
    const hierarchy = typeof content.requiredHierarchy === "string" ? content.requiredHierarchy.trim() : "";
    if (hierarchy) parts.push(`--- Verified when ${row.id} was approved ---
${hierarchy}`);
  }
  return parts.join("\n\n");
}
__name(getVerifiedStructure, "getVerifiedStructure");
async function getLatestSubmittedCode(env) {
  const approved = await env.DB.prepare(
    "SELECT id FROM missions WHERE status = 'approved' ORDER BY number DESC"
  ).all();
  const rows = approved.results || [];
  if (!rows.length) {
    return { ok: false, reason: "No approved mission exists yet, so there is no real submitted code to ground a new mission in." };
  }
  for (const row of rows) {
    const sub = await env.DB.prepare(
      "SELECT fields_json FROM submissions WHERE mission_id = ? AND verdict = 'approved' ORDER BY attempt_number DESC LIMIT 1"
    ).bind(row.id).first();
    if (!sub) continue;
    let fields;
    try {
      ...
      return { ok: false, reason: `The approved submission recorded for ${row.id} can't be read \u2014 its saved data isn't valid JSON. Fix that submission record before planning can continue.` };
    }
    const code = typeof fields.code === "string" ? fields.code.trim() : "";
    if (code) {
      return { ok: true, code, sourceMissionId: row.id };
    }
  }
  return { ok: false, reason: "No approved mission has any submitted code on record yet, so there is no real code to ground a new mission in." };
}
__name(getLatestSubmittedCode, "getLatestSubmittedCode");
async function planNextMission(env) {
  ...
  if (!codeResult.ok) {
    await logEvent(env, "plan_error", `Mission planning stopped before writing anything \u2014 ${codeResult.reason}`);
    throw new Error(codeResult.reason);
  }
  const verifiedStructure = await getVerifiedStructure(env);
...
The next mission should be numbered ${nextNumber}, with id "V1-M${String(nextNumber).padStart(2, "0")}".`;
  const raw = await callClaude(env, {
    model: env.MODEL_STRONG,
    system: planningSystemPrompt(codeResult.code, verifiedStructure),
  ...
  try {
    const qaRaw = await callClaude(env, {
      model: env.MODEL_STRONG,
      system: qaSystemPrompt(codeResult.code, verifiedStructure),
